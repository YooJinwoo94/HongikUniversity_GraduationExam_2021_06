...
public class Boss01Pattern07 : Action
{
    ...
    {
        if (numOfPattern.Value != 7) return TaskStatus.Failure;

        if (hpPostionScript.deadOrLive == 1) return TaskStatus.Failure;

        if ((aniStart == false) && Vector3.Distance(transform.position, target.Value.position) < checkPatternForEnemyScript.bossAttackDistancePattern[7])
        {
            ischaseStart = false;
        ...
        if ((aniStart == true) && (startTime + 0.4f < Time.time)) weaponColliderConScript.weaponColliderOn(0);
